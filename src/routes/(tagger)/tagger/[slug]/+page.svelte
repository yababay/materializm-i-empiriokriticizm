<script lang="ts">
    import { idBySlug, markdownByPath } from '$lib/assets/md';
    ...
    const ID_PREFIX = idBySlug(slug)

    const queryParagraphs = (quot = false) => Array.from(document.querySelectorAll(`article ${quot ? 'blockquote' : ''} p`) as NodeListOf<HTMLParagraphElement>)

    let paragraphCount = 0 //: () => queryParagraphs().length
    let quotationsCount = 0 //: () => queryParagraphs(true).length

    onMount(() => {

        paragraphCount = queryParagraphs().length
        quotationsCount = queryParagraphs(true).length
        
        const i = +(localStorage.getItem('previous') || 1)
        const paragraphs = queryParagraphs() // Array.from(document.querySelectorAll('article p') as NodeListOf<HTMLParagraphElement>)
    ...
    const findParagraphId = () => {
        const paragraphs = queryParagraphs() //document.querySelectorAll('article p') as NodeListOf<HTMLParagraphElement>
...
<form method="post" on:submit={submit} class="position-relative">
    <div class="row h-100">
        <div class="col-6 d-flex flex-column align-items-center justify-content-center">
            ...
            <Submit />
            <p>Количество параграфов: {paragraphCount}</p>
            <p>В том числе цитат: {quotationsCount}</p>
        </div>
        <div class="col-6 pe-5" bind:this={article}>
            <Article>
                {@html markdownByPath(slug)}
            </Article>
        </div>
    </div>
</form>
